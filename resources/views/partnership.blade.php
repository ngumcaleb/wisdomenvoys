...
<section class="relative min-h-[400px] sm:min-h-[460px] md:min-h-[540px] lg:min-h-[600px] flex items-end sm:items-center overflow-hidden hero-bg" style="background-image: url('{{ $heroBg }}')">
    ...
    <div class="absolute top-1/2 right-0 -translate-y-1/2 translate-x-1/4 w-[300px] sm:w-[400px] md:w-[500px] h-[300px] sm:h-[400px] md:h-[500px] border border-white/[0.05] rounded-full pointer-events-none"></div>
    <div class="absolute top-1/2 right-8 -translate-y-1/2 w-[200px] sm:w-[280px] md:w-[360px] h-[200px] sm:h-[280px] md:h-[360px] border border-white/[0.08] rounded-full pointer-events-none"></div>
    ...
    <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4 sm:gap-5 md:gap-8 mb-10 sm:mb-12 md:mb-16 lg:mb-20">
        <div class="max-w-xl">
            <div class="inline-flex items-center gap-2 bg-primary/8 rounded-full px-3.5 sm:px-4 py-1.5 sm:py-2 mb-4 sm:mb-5">
                ...
                <span class="font-headline text-[10px] sm:text-[11px] md:text-xs font-bold uppercase tracking-[0.15em] text-primary">OUR GLOBAL MISSION</span>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
        @foreach($impact as $item)
            <div class="p-6 sm:p-7 md:p-8 bg-white rounded-[20px] sm:rounded-[24px] border border-outline-variant/50 shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex flex-col items-center text-center group hover:shadow-[0_8px_40px_rgba(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-500">
                <div class="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-primary/10 flex items-center justify-center mb-5 sm:mb-6 group-hover:bg-primary group-hover:shadow-lg group-hover:shadow-primary/20 transition-all duration-500">
                    <span class="material-symbols-outlined text-primary text-[24px] sm:text-[26px] group-hover:text-white transition-colors duration-500">{{ $item['icon'] }}</span>
                </div>
    ...
    <div class="max-w-[1280px] mx-auto px-5 sm:px-8 md:px-10">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-stretch">
            ...
            <div class="lg:col-span-7 bg-white p-8 sm:p-10 md:p-12 shadow-[0_2px_20px_rgba(0,0,0,0.04)] relative overflow-hidden flex flex-col justify-center rounded-[20px] sm:rounded-[24px] border border-outline-variant/50">
                <div class="absolute top-0 right-0 w-24 sm:w-32 h-24 sm:h-32 bg-primary/5 rounded-bl-full pointer-events-none"></div>
                <span class="material-symbols-outlined text-primary/10 text-[40px] sm:text-[60px] absolute top-6 right-6 sm:top-8 sm:right-8">format_quote</span>
            ...
            <div class="lg:col-span-5 grid grid-rows-2 gap-4 sm:gap-5 md:gap-6">
                @foreach($stats as $stat)
                    <div class="p-8 sm:p-9 md:p-10 text-white flex flex-col justify-end rounded-[20px] sm:rounded-[24px] {{ $loop->first ? 'bg-primary shadow-lg shadow-primary/15' : 'bg-on-secondary-fixed' }}">
                        <span class="font-headline text-[36px] sm:text-[42px] md:text-[48px] font-extrabold block mb-1 sm:mb-2">{{ $stat['value'] }}</span>
                        <p class="font-headline text-[10px] sm:text-[11px] md:text-xs font-bold uppercase tracking-[0.1em] opacity-80">{{ $stat['label'] }}</p>
                    </div>
                @endforeach
            </div>
        </div>
...
<section class="py-14 sm:py-18 md:py-24 lg:py-28 max-w-[1280px] mx-auto px-5 sm:px-8 md:px-10" id="join-form">
    <div class="flex flex-col lg:flex-row gap-10 sm:gap-12 md:gap-16">
        <div class="lg:w-1/2">
            <div class="inline-flex items-center gap-2 bg-primary/8 rounded-full px-3.5 sm:px-4 py-1.5 sm:py-2 mb-4 sm:mb-5">
                <span class="w-1.5 h-1.5 bg-primary rounded-full animate-pulse"></span>
                <span class="font-headline text-[10px] sm:text-[11px] md:text-xs font-bold uppercase tracking-[0.15em] text-primary">JOIN US</span>
            </div>
            ...
            <ul class="space-y-4 sm:space-y-5">
                @foreach(['Access to exclusive partner-only webinars and spiritual resources.', 'Monthly prayer focus for your family and business.', 'First-hand updates on global outreach missions and school initiatives.'] as $benefit)
                    <li class="flex gap-3 sm:gap-4 items-start">
                        <span class="w-6 h-6 sm:w-7 sm:h-7 bg-primary/10 rounded-full flex items-center justify-center shrink-0 mt-0.5">
                            <span class="material-symbols-outlined text-primary text-[14px] sm:text-[16px]">check</span>
                        </span>
